...
class Minimizer_function:
    ...
    def minimizer(self, f, x0, step_len=None, obj_tol=1e-12, param_tol=1e-8, max_cond=1e+10, hessian=False, wolfe_backtracking=False):
        f_history = []
        x_history = []
        f.hessian = hessian
        status = False
        if hessian:
            ...
            for i in range(1, max_iter+1):
                ...
                if cond > max_cond:
                    print('Error, inversion condition number greater then max condition number!')
                    status = False
                    break
                ...
                # Update next function location and function value
                x = x.copy()
                x += p_k
                f_x, g_x, h_x = f.func(x)
                x_history.append(x)
                ...
                if len(f_history) and len(x_history) > 1:
                    ...
                    if abs(function_values_diff) < obj_tol or function_location_diff < param_tol:
                        print('Achieved numeric tolerance for successful termination!')
                        x_history.pop()
                        f_history.pop()
                        status = True
                        break
                if abs(f_x) >= 1e+4:
                    print('Error! Function value is too large')
                    x_history.pop()
                    f_history.pop()
                    status = False
                    break
            ...
            for i in range(1, max_iter+1):
                ...
                print(f'Iteration number {i}:\nFunction location is: {x.T}\nFunction value is: {f_x}')
                print('-'*45)
        self.function_dict = {'Minimization method': function_method, 'Function location list': np.vstack(x_history).T, 'Function value list': f_history}
