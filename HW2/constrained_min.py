...
class Minimizer_function_constrained:
    ...
    def interior_pt(self, f_0, x0, ineq_constraints=None, eq_constraints_mat=None, eq_constraints_rhs=None, max_outer_iterations=None, obj_tol=1e-12, param_tol=1e-8, max_m=False, d=1):
        ...
        f_x0 = f_0.func(x0)[0]
        x_history.append(x0)
        nu_history.append(self.nu)
        f_history.append(f_x0)
        ...
        while outer_iteration < max_outer_iterations:
            l_min = Minimizer_function()
            x_nu = l_min.minimizer(f=lagrangian_func, x0=x0, hessian=True, print_status=False, wolfe_backtracking=True)[0]
            x0 = x_nu
            x = x0[:self.x_dim]
            ...
            print('-' * 45)
            outer_iteration += 1
        self.function_dict = {'Function location list': x_history, 'Function value list': d*np.array(f_history)}
        return x_history[-1], d*f_history[-1], nu_history[-1]
